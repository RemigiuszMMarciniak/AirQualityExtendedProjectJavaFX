...
public class MainController {
    ...
    @FXML
    private TextField mainTextFieldDateFrom;

    @FXML
    private TextField mainTextFieldTimeFrom;

    @FXML
    private TextField mainTextFieldDateTo;

    @FXML
    private TextField mainTextFieldTimeTo;
...
//                xAxis.setStyle("-fx-text-fill: white;");
    ...
    ArrayList<XYChart.Series> observedSeries = new ArrayList<>();
    boolean areVeryGoodValuesLoaded = false;
    @FXML
    void observe(ActionEvent event) {

        String cityName = mainTextFieldObservedCity.getText();
        System.out.println(cityName);
        String dateFrom = mainTextFieldDateFrom.getText();
        System.out.println(dateFrom);
        String timeFrom = mainTextFieldTimeFrom.getText();
        String dateTo = mainTextFieldDateTo.getText();
        String timeTo = mainTextFieldTimeTo.getText();
        ...
        StringBuffer response = new StringBuffer();
        String url = "https://api.openaq.org/v1/measurements?city=" + cityNamePrepared + "&parameter[]=pm25&parameter[]=pm10" +
                "&parameter[]=so2&parameter[]=no2&parameter[]=o3&parameter[]=co&date_from=" + dateFrom + "T" + timeFrom + "&date_to=" +
                dateTo + "T" + timeTo;
        ...
        System.out.println("created axes");
        ...
        if(!areVeryGoodValuesLoaded){
            ...
            System.out.println("data added to series");

            series.setName("Very good values");

            observedSeries.add(series);
            areVeryGoodValuesLoaded=true;
        }
        ...
        String date;
        if (results.getResults().isEmpty()) {
            System.out.println("no data with such date");

        } else {
            XYChart.Series seriesFound;

            date = results.getResults().get(0).getDate().getUtc();
            String datePrepared = date.substring(0, 10) + " " + date.substring(11, 19);

            System.out.println(datePrepared);

            ArrayList<String> valuesToBeShown = parametersToBeShown(results);

            System.out.println(valuesToBeShown);


            //found values


            seriesFound = new XYChart.Series();
            seriesFound.setName(dateFrom+"T"+timeTo+","+dateTo+"T"+timeTo);
            ...
            observedSeries.add(seriesFound);

            mainPaneView1.getChildren().clear();

            for (XYChart.Series s:observedSeries) {
                barChart.getData().add(s);

            }

            mainPaneView1.getChildren().add(barChart);
        ...
        void initialize () {
            ...
            assert mainTextFieldDateFrom != null : "fx:id=\"mainTextFieldDateFrom\" was not injected: check your FXML file 'main.fxml'.";
            assert mainTextFieldTimeFrom != null : "fx:id=\"mainTextFieldTimeFrom\" was not injected: check your FXML file 'main.fxml'.";
            assert mainTextFieldDateTo != null : "fx:id=\"mainTextFieldDateTo\" was not injected: check your FXML file 'main.fxml'.";
            assert mainTextFieldTimeTo != null : "fx:id=\"mainTextFieldTimeTo\" was not injected: check your FXML file 'main.fxml'.";
